...
const ShowInvoiceForm: React.FC<ShowInvoiceFormProps> = ({
  invoice,
  onClose,
}) => {
      ...
      <div className="bg-white w-full max-w-xl rounded-2xl shadow-xl overflow-hidden animate-slideUp">
        ...
        <div className="px-6 py-5 space-y-5">
          <Section title="Thông tin chung">
            ...
            <FormRow
              label="Số tiền"
              value={invoice.amount.toLocaleString() + " đ"}
            />
          ...
          <Section title="Gói dịch vụ">
            ...
            <FormRow
              label="Thời hạn"
              value={`${invoice.durationMonths} tháng`}
            />
            <FormRow
              label="Ngày hết hạn"
            ...
            <FormRow
              label="Cập nhật cuối"
              value={invoice.lastModified.substring(0, 10)}
            />
          </Section>
...
const Section = ({
  ...
  <div>
    ...
    <div className="space-y-2 bg-gray-50 p-3 rounded-xl border">{children}</div>
  </div>
);
